...
class Triplet:
    ...
    def add(self, Triplet):
        ...
        '''
        if(self.__type == 'P' and Triplet.get_t() == 'V'):
            new_t = 'P'
        elif(not self.check_type('P', Triplet.get_t())):
            print('Interdit')
            return None
        else:
            new_t = self.__type
    ...
    def len(self):
        if (not self.check_type('P', Triplet.get_t) or not self.check_type('C', Triplet.get_t)):
            ...
            sum = 0
            sum += triplet.pow(self.__x, 2)
            sum += triplet.pow(self.__y, 2)
            sum += triplet.pow(self.__z, 2)
            return triplet.sqrt(sum)
    
    def get_this(self):
        return [self.__x,self.__y,self.__z]
    
    def infos(self):
        print(f"{float(self.__x)} {float(self.__y)} {float(self.__z)}", end=' ')

    def show(self):
        print(f"{self.__type} {float(self.__x)} {float(self.__y)} {float(self.__z)}")

    def __str__(self):
        return f"{self.__type} {float(self.__x)} {float(self.__y)} {float(self.__z)}"
